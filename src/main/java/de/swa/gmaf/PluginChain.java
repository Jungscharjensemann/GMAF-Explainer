...
import de.swa.gmaf.plugin.fusion.FeatureFusionPlugin;
	...
	public PluginChain(Vector<String> pluginClasses) {
		for (String s : pluginClasses) {
			try {
				...
				LogPanel.getCurrentInstance().addToLog("added plugin " + s);
			} catch (Exception x) {
							...
							for (TechnicalAttribute ta : n.getTechnicalAttributes()) {
								...
								try {
									...
									if (fv.getNodes().size() > max) {
										LogPanel.getCurrentInstance()
												.addToLog("max number of nodes reached - returning");
										return;
									}
								} catch (Exception x) {
									x.printStackTrace();
