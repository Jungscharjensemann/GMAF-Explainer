	...
	public static GraphCode subtract(GraphCode gc1, GraphCode gc2) {
		...
		for (String s : dictionary) {
			for (String t : dictionary) {
				int i = 0;
				try {
					i = gc1.getEdgeValueForTerms(s, t);
				} catch (Exception x) {
					x.printStackTrace();
				}
...
//		stopWords.add("flash photography");
	...
	public static GraphCode getSimilarities(Vector<GraphCode> gcs) {
		...
		for(GraphCode gc : gcs) {
			unionDic.retainAll(gc.getDictionary());
		}
		Vector<String> dictionary = new Vector<>(unionDic);
	...
	public static GraphCode getDifferences(Vector<GraphCode> gcs) {
		...
		Sets.SetView<String> setView = Sets.difference(new LinkedHashSet<>(union.getDictionary()), new LinkedHashSet<>(unionDic));
		Vector<String> dictionary = new Vector<>(setView);
