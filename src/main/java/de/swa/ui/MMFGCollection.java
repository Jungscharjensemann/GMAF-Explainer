			...
			for (String path : paths) {
				...
				try {
					...
					for (File fi : fs) {
						...
						if (ProgressFrame.getInstance() != null)
							ProgressFrame.getInstance().setProgress(progress,
									"loading " + path + " > " + fi.getName() + " (" + count + " / " + fs.length + ")");
						try {
...
//			ex.printStackTrace();
	...
	public void query(GraphCode gcQuery) {
		...
		if (AssetListPanel.getCurrentInstance() != null)
